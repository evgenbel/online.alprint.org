...
class ModelCatalogProduct extends Model {
	...
	public function add($data)
	{
		...
		if (isset($materials[-1])){
			$this->load->model('module/materials');
			$mid = $this->model_module_materials->addOther($data['material_other']);
		}
		$product_id = 0;

		$sql = "INSERT " . DB_PREFIX . "product_customer
		...
		foreach($materials as $k=>$mat){
			...
			$sql = "INSERT INTO " . DB_PREFIX . "product_customer_materials(product_id, material_id)
				VALUES('".(int)$product_id."','" . (int)$mat . "')";
			$this->db->query($sql);
		}

		$this->sendNotifyNewEtik($data);

	}

	public function sendNotifyNewEtik($data)
	{
